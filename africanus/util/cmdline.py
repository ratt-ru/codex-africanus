...
def parse_python_assigns(assign_str):
    ...
    if isinstance(stmt_value, ast.Call):
      ...
      if func_name not in _BUILTIN_WHITELIST:
        raise ValueError(
          ...
          "Available builtins: '%s'" % (func_name, assign_str, list(_BUILTIN_WHITELIST))
        )
  ...
  for i, stmt in enumerate(stmts):
    if not isinstance(stmt, ast.Assign):
      raise ValueError(
        "Statement %d in '%s' is not a " "variable assignment." % (i, assign_str)
      )
    ...
    for target in stmt.targets:
      ...
      # (a, b) = 2
        ...
        raise TypeError(
          "'%s' types are not supported" "as assignment targets." % type(target)
        )
