...
def freeze(arg):
  ...
  if isinstance(arg, set):
    ...
    if arg.nbytes > 10:
      warn(f"freezing ndarray of size {arg.nbytes} " f" is probably inefficient")
...
class Multiton(type):
  ...
  def __call__(cls, *args, **kwargs):
    ...
    positional_in_kwargs = [
      ...
      if p.kind == p.POSITIONAL_OR_KEYWORD and p.default == p.empty and p.name in kwargs
    ]
  ...
  def __lazy_obj_from_args__(cls, proxy):
    ...
    with proxy.__lazy_lock__:
      ...
      try:
        ...
        lazy_obj = proxy.__lazy_fn__(*proxy.__lazy_args__, **proxy.__lazy_kwargs__)
