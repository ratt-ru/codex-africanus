...
def parse_corrs(corrs_string):
  ...
  if not isinstance(corrs, list) or not all(isinstance(c, str) for c in corrs):
    raise RimeParseError(
      f"Correlation specification must be of the form " f"[XX,XY,YX,YY]. Got {corrs}."
    )
...
class RimeSpecification:
  ...
  **Custom Terms**
  ...
  def __eq__(self, rhs):
    return isinstance(rhs, RimeSpecification) and self._saved_args == rhs._saved_args
  ...
  def __init__(self, specification, terms=None, transformers=None):
    ...
    if not terms:
      ...
      raise TypeError(
        f"terms: {terms} must be a dictionary or " f"an iterable of (key, value) pairs"
      )
    ...
    for cls, cfg in zip(term_types, term_cfgs):
      ...
      for a, p in list(init_sig.parameters.items())[1:]:
        if p.kind not in {p.POSITIONAL_ONLY, p.POSITIONAL_OR_KEYWORD}:
          raise RimeSpecificationError(
            f"{cls}.__init__{init_sig} may not contain " f"*args or **kwargs"
          )
