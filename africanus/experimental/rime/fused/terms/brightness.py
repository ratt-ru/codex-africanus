  ...
  def corr_convert(typingctx, spectral_model, source_index, chan_index):
    ...
    for corr in corr_schema:
      try:
        ...
        raise ValueError(f"No conversion schema " f"registered for correlation {corr}")
...
class Brightness(Term):
  ...
  def init_fields(
    ...
    def brightness(init_state, stokes, spi, ref_freq, chan_freq, spi_base="standard"):
      nsrc, nstokes = stokes.shape
      (nchan,) = chan_freq.shape
      nspi = spi.shape[1]

      if nstokes != expected_nstokes:
        raise ValueError("corr_schema stokes don't match " "provided number of stokes")
      ...
      if (spi_base.startswith("[") and spi_base.endswith("]")) or (
        ...
        if len(list_spi_base) != nstokes:
          raise ValueError(
            "List of spectral bases must equal " "number of stokes parameters"
          )
      else:
