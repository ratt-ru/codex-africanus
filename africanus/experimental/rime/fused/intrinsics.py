...
def diag_full(lhs, rhs):
  return (lhs[0] * rhs[0], lhs[0] * rhs[1], lhs[1] * rhs[2], lhs[1] * rhs[3])
...
def full_diag(lhs, rhs):
  return (lhs[0] * rhs[0], lhs[1] * rhs[1], lhs[2] * rhs[0], lhs[3] * rhs[1])
...
def hermitian_full(jones):
  return (np.conj(jones[0]), np.conj(jones[2]), np.conj(jones[1]), np.conj(jones[3]))
...
def unify_jones_terms(typingctx, lhs, rhs):
  ...
  try:
    ...
    raise TypingError(f"{lhs} or {rhs} has no " f"entry in the {corr_map} " f"mapping")
...
class IntrinsicFactory:
  ...
  def _resolve_arg_dependencies(self):
    ...
    for k, v in argdeps.optional.items():
      ...
      if len(defaults) != 1:
        raise ValueError(
          f"Multiple terms: {argdeps.terms} have "
  ...
  def pack_optionals_and_indices_fn(self):
    argdeps = self.argdeps
    out_names = (
      argdeps.names + tuple(argdeps.optional_defaults.keys()) + tuple(argdeps.KEY_ARGS)
    )
    ...
    def pack_index(typingctx, args):
      ...
      if tuple(key_types.keys()) != argdeps.KEY_ARGS:
        raise RuntimeError(f"{tuple(key_types.keys())} != {argdeps.KEY_ARGS}")

      rvt = typingctx.resolve_value_type_prefer_literal
      optionals = [(n, rvt(d), d) for n, d in argdeps.optional_defaults.items()]
      optional_types = tuple(p[1] for p in optionals)

      return_type = types.Tuple(args.types + optional_types + tuple(key_types.values()))
      ...
      def codegen(context, builder, signature, args):
        ...
        # the new tuple
        fn_args = [
          builder.extract_value(args[0], arg_info[a][1]) for a in argdeps.REQUIRED_ARGS
        ]
        fn_arg_types = tuple(arg_info[k][0] for k in argdeps.REQUIRED_ARGS)
  ...
  def pack_transformed_fn(self, arg_names):
    ...
    out_names = arg_names + tuple(o for t in transformers for o in t.OUTPUTS)
    ...
    def pack_transformed(typingctx, args):
      ...
      return_type = types.Tuple(args.types + tuple(transform_output_types))

      # Sanity check
      if len(return_type) != len(out_names):
        raise TypingError(
          f"len(return_type): {len(return_type)} != "
      ...
      def codegen(context, builder, signature, args):
        ...
        for transformer in transformers:
          ...
          for name, default in transformer.KWARGS.items():
            ...
            default_value = context.get_constant_generic(builder, default_typ, default)
          ...
          if single_return:
            ...
            for j, o in enumerate(transformer.OUTPUTS):
              ...
              ret_tuple = builder.insert_value(ret_tuple, element, i + n)
              i += 1
  ...
  def term_state_fn(self, arg_names):
    ...
    def term_state(typingctx, args):
      ...
      for term in argdeps.terms:
        ...
        fields, constructor = term.init_fields(typingctx, init_state_type, **arg_types)
      ...
      def codegen(context, builder, signature, args):
        ...
        for term in argdeps.terms:
          ...
          for typ, i in zip(arg_types, arg_index):
            if isinstance(typ, types.Omitted):
              ...
              const = context.get_constant_generic(builder, const_type, typ.value)
              cargs.append(const)
        ...
        for ti in range(len(argdeps.terms)):
          ...
          return_value = context.compile_internal(
            builder, constructors[ti], constructor_sig, constructor_args[ti]
          )
  ...
  def term_sampler_fn(self):
    ...
    def term_sampler(typingctx, state, s, r, t, f1, f2, a1, a2, c):
      ...
      for typ in sampler_return_types[1:]:
        sampler_ret_type = unify_jones_terms(typingctx, sampler_ret_type, typ)
      ...
      def codegen(context, builder, signature, args):
        ...
        for jrt, j in zip(sampler_return_types[1:], jones[1:]):
          ...
          jones_mul_typ = unify_jones_terms(context.typing_context, prev_t, jrt)
          jones_sig = jones_mul_typ(prev_t, jrt)
          prev = context.compile_internal(builder, jones_mul, jones_sig, [prev, j])
