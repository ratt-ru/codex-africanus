    ...
    def pack_optionals_and_indices_fn(self):
        ...
        def pack_index(typingctx, args):
            ...
            return_type = types.Tuple(
                args.types + optional_types + tuple(key_types.values())
            )
            ...
            def codegen(context, builder, signature, args):
                ...
                # the new tuple
                fn_args = [
                    builder.extract_value(args[0], arg_info[a][1])
        ...
        def term_sampler(typingctx, state, s, r, t, f1, f2, a1, a2, c):
            ...
            def codegen(context, builder, signature, args):
                ...
                for jrt, j in zip(sampler_return_types[1:], jones[1:]):
                    ...
                    jones_mul_typ = unify_jones_terms(
                        context.typing_context, prev_t, jrt
                    )
                    jones_sig = jones_mul_typ(prev_t, jrt)
                    prev = context.compile_internal(
                        builder, jones_mul, jones_sig, [prev, j]
                    )
