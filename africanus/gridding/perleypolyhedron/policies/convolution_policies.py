...
def convolve_1d_axisymmetric_unpacked_scatter(scaled_u, scaled_v, scaled_w,
                                              npix,
                                              grid,
                                              vis,
                                              r,c,
                                              ...
                                              policy_type):
    '''
    Convolution policy for a 1D axisymmetric unpacked AA kernel (gridding kernel)
    ...
    for tv in range(convolution_kernel_width):
        conv_v = convolution_kernel[(tv + 1) * convolution_kernel_oversampling + frac_v]
...
def convolve_1d_axisymmetric_packed_scatter(scaled_u, scaled_v, scaled_w,
    ...
    cw = 0.0
    for tv in range(convolution_kernel_width):
        conv_v = convolution_kernel[tv + frac_offset_v + frac_v * (convolution_kernel_width+2)]
        grid_v_lookup = disc_v + tv - convolution_kernel_width//2
        for tu in range(convolution_kernel_width):
            conv_u = convolution_kernel[tu + frac_offset_u + frac_u * (convolution_kernel_width+2)]
            grid_u_lookup = disc_u + tu - convolution_kernel_width//2
            if (grid_v_lookup >= 0 and grid_v_lookup < npix and 
                grid_u_lookup >= 0 and grid_u_lookup < npix):
                grid[grid_v_lookup, grid_u_lookup] += \
...
def convolve_1d_axisymmetric_unpacked_gather(scaled_u, scaled_v, scaled_w,
                                             npix,
                                             grid,
                                             vis,
                                             r,c,
                                             ...
                                             policy_type):
    '''
    Convolution policy for a 1D axisymmetric unpacked AA kernel (degridding kernel)
    @scaled_u: simularity theorem and lambda scaled u
    @scaled_v: simularity theorem and lambda scaled v
    @scaled_w: simularity theorem and lambda scaled w
    @npix: number of pixels per axis
    ...
    for tv in range(convolution_kernel_width):
        ...
        for tu in range(convolution_kernel_width):
            ...
            if (grid_v_lookup >= 0 and grid_v_lookup < npix and 
                grid_u_lookup >= 0 and grid_u_lookup < npix):
                scp.stokes2corr(grid[grid_v_lookup, grid_u_lookup] * conv_v * conv_u,
                                vis[r,c,:],
                                policy_type=literally(stokes_conversion_policy))
                cw += conv_v * conv_u
    vis[r,c,:] /= cw + 1.0e-8
