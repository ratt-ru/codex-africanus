...
def __degrid(uvw,
    ...
    nchan = lambdas.size
    nrow = uvw.shape[0]
    ncorr = stokes_conversion_policies.ncorr_outpy(policy_type=stokes_conversion_policy)()
...
def degridder(uvw,
    ...
    @rowparallel: adds additional threading per row per chunk. This may be
                  ...
                  documentation http://numba.pydata.org/numba-doc/0.46.0/user/threading-layer.html
                  Must set:
                  from numba import config # have to be openmp to support nested parallelism
    ...
    if gridstack.ndim != 4 or gridstack.shape[0] != image_centres.shape[0]:
        raise ValueError("Grid stack must be nfacet x nband x yy x xx and match number of image centres")
    vis = da.blockwise(__degrid, ("row","chan","corr"),
                       uvw, ("row", "uvw"),
                       ...
                       new_axes={"corr": stokes_conversion_policies.ncorr_outpy(policy_type=stokes_conversion_policy)()},
                       dtype=vis_dtype,
                       meta=np.empty((0,0,0), dtype=vis_dtype) # row, chan, correlation product as per MSv2 spec
                      )
    return vis
