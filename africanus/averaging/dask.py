...
from africanus.averaging.bda_mapping import bda_mapper as np_bda_mapper
...
def tc_chan_metadata(row_chan_arrays, chan_arrays, chan_bin_size):
  ...
  layers = {
    (name, i): (np_tc_channel_mapper, c, chan_bin_size)
...
def _tc_row_average_wrapper(
  row_meta, ant1, ant2, flag_row, time_centroid, exposure, uvw, weight, sigma
):
  return np_tc_row_average(
    row_meta,
    ant1,
    ant2,
    flag_row,
...
def tc_row_average(
  ...
  tuple_gets = [
    None if a is None else _getitem_row(avg, i, a, dims)
...
def tc_row_chan_average(
  ...
  if isinstance(visibilities, (tuple, list)):
    if not all(isinstance(a, da.Array) for a in visibilities):
      raise ValueError("Visibility tuple must exclusively " "contain dask arrays")

    have_vis_tuple = True
    nvis_elements = len(visibilities)
    meta = np.empty((0, 0, 0), visibilities[0].dtype)

    visibilities = da.blockwise(
      lambda *a: a,
      _row_chan_avg_dims,
      *[elem for a in visibilities for elem in (a, _row_chan_avg_dims)],
      meta=meta,
    )

  avg = da.blockwise(
    np_tc_row_chan_average,
    _row_chan_avg_dims,
    row_meta,
    ("row",),
    chan_meta,
    ...
    meta=np.empty((0,) * len(_row_chan_avg_dims), dtype=object),
    dtype=object,
  )

  tuple_gets = (
    None if a is None else _getitem_row_chan(avg, i, a.dtype)
    for i, a in enumerate([visibilities, flag, weight_spectrum, sigma_spectrum])
  )
...
def _getitem_chan(avg, idx, dtype):
  ...
  graph = HighLevelGraph.from_collections(name, layers, (avg,))
  return da.Array(
    graph, name, avg.chunks, meta=np.empty((0,), dtype=dtype), dtype=dtype
  )
...
def tc_chan_average(
  ...
  avg = da.blockwise(
    ...
    cdim,
    chan_freq,
    None if chan_freq is None else cdim,
    chan_width,
    None if chan_width is None else cdim,
    effective_bw,
    None if effective_bw is None else cdim,
    resolution,
    None if resolution is None else cdim,
    adjust_chunks=adjust_chunks,
    meta=np.empty((0,), dtype=object),
    dtype=object,
  )
...
def time_and_channel(
  ...
  row_chan_data = tc_row_chan_average(
    row_meta,
    chan_meta,
    flag_row=flag_row,
    weight=weight,
  ...
  # Merge output tuples
  return TcAverageOutput(
    _getitem_row(row_meta, 1, time, ("row",)),
    _getitem_row(row_meta, 2, interval, ("row",)),
    (_getitem_row(row_meta, 3, flag_row, ("row",)) if flag_row is not None else None),
    row_data.antenna1,
    row_data.antenna2,
...
def _bda_row_average_wrapper(
  meta, ant1, ant2, flag_row, time_centroid, exposure, uvw, weight, sigma
):
  return np_bda_row_avg(
    meta,
    ant1,
    ant2,
    flag_row,
...
def _ragged_row_chan_getitem(avg, idx, meta):
  ...
  if isinstance(data, tuple):
    return tuple(
      {
        "r%d" % (r + 1): d[None, s:e, ...]
        for r, (s, e) in enumerate(zip(meta.offsets[:-1], meta.offsets[1:]))
      }
      for d in data
    )

  return {
    "r%d" % (r + 1): data[None, s:e, ...]
    for r, (s, e) in enumerate(zip(meta.offsets[:-1], meta.offsets[1:]))
  }
...
def _bda_getitem_row_chan(avg, idx, dtype, format, avg_meta, nchan):
  ...
  if format == "flat":
    ...
    layers = db.blockwise(
      ...
      ("row",),
      new_axes=new_axes,
      numblocks={avg.name: avg.numblocks, avg_meta.name: avg_meta.numblocks},
    )
...
def bda_row_average(
  ...
  tuple_gets = [
    None if a is None else _bda_getitem_row(avg, i, a, dims, meta, format=format)
    for i, (a, dims) in enumerate(out_args)
  ]
...
def _bda_row_chan_average_wrapper(
  avg_meta, flag_row, weight, vis, flag, weight_spectrum, sigma_spectrum
):
...
def bda_row_chan_average(
  ...
  """Average (row,chan,corr)-based dask arrays"""
  if all(v is None for v in (visibilities, flag, weight_spectrum, sigma_spectrum)):
  ...
  if isinstance(visibilities, da.Array):
    ...
    visibilities = da.blockwise(
      lambda *a: a,
      _row_chan_avg_dims,
      *[elem for a in visibilities for elem in (a, vis_dims)],
      meta=meta,
    )
  ...
  tuple_gets = (
    None
    if a is None
    else _bda_getitem_row_chan(avg, i, a.dtype, format, avg_meta, nchan)
    for i, a in enumerate([visibilities, flag, weight_spectrum, sigma_spectrum])
  )
  ...
  if have_vis_tuple:
    ...
    for v in range(nvis_elements):
      v = da.blockwise(
        getitem, bda_dims, vis_array, bda_dims, v, None, dtype=vis_array.dtype
      )
      tuple_vis.append(v)
...
def bda(
  ...
  if not len(chan_width.chunks[0]) == 1:
    raise ValueError("Chunking in channel is not " "currently supported.")
  ...
  # Average channel data
  row_chan_data = bda_row_chan_average(
    meta,
    flag_row=flag_row,
    weight=weight,
    visibilities=visibilities,
  ...
  fake_map = da.zeros(
    (time.shape[0], chan_width.shape[0]),
  ...
  meta_time = _bda_getitem_row(meta, 3, time, ("row",), meta, format=format)
  meta_interval = _bda_getitem_row(meta, 4, interval, ("row",), meta, format=format)
  meta_chan_width = _bda_getitem_row(meta, 5, chan_width, ("row",), meta)
  meta_flag_row = (
    _bda_getitem_row(meta, 6, flag_row, ("row",), meta, format=format)
    if flag_row is not None
    else None
  )

  # Merge output tuples
  return BDAAverageOutput(
    meta_map,
    meta_offsets,
    meta_decorr_cw,
    meta_time,
