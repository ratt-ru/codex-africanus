...
def chan_width(nchan):
    return np.full(nchan, (2 * 0.856e9 - 0.856e9) / nchan)


@pytest.fixture(scope="session")
def chan_freq(chan_width):
    return 0.856e9 + np.cumsum(np.concatenate([[0], chan_width[1:]]))
...
def synthesized_uvw(ants, time, phase_dir, auto_correlations):
    ...
    ref_dir = dm.direction("j2000", q(phase_dir[0], "rad"), q(phase_dir[1], "rad"))
    ox, oy, oz = ants[0]
    ...
    ant1, ant2 = np.triu_indices(ants.shape[0], 0 if auto_correlations else 1)
    ...
    for ti, t in enumerate(time):
        ...
        for ai, (x, y, z) in enumerate(ants):
            bl = dm.baseline("ITRF", q([x, ox], "m"), q([y, oy], "m"), q([z, oz], "m"))
...
def test_bda_mapper(
    time, synthesized_uvw, interval, chan_freq, chan_width, decorrelation, min_nchan
):
    time = np.unique(time)
...
def test_bda_binner(time, interval, synthesized_uvw, ref_freq, chan_freq, chan_width):
    time = np.unique(time)
    ant1, ant2, uvw = synthesized_uvw
