...
@pytest.fixture
def ant1():
    return np.asarray([0, 0, 1, 0, 0, 1, 2, 0, 0, 1], dtype=np.int32)


@pytest.fixture
def ant2():
    return np.asarray([1, 2, 2, 0, 1, 2, 3, 0, 1, 2], dtype=np.int32)
...
@pytest.fixture
def frequency():
    return np.linspace(0.856, 2 * 0.856e9, nchan)


@pytest.fixture
def chan_width():
    return np.full(nchan, 0.856e9 / nchan)
...
def vis():
    def _vis(row, chan, fcorrs):
        flat_vis = (
            np.arange(row * chan * fcorrs, dtype=np.float64)
...
def test_averager(
    ...
    time_bl_row_map = _gen_testing_lookup(
        time_centroid, exposure, ant1, ant2, flag_row, time_bin_secs, row_meta
    )
    ...
    for orow, idx in enumerate(eff_idx):
        for ch, (cs, ce) in enumerate(zip(chan_ranges[:-1], chan_ranges[1:])):
            for corr in range(ncorr):
                ...
                assert_array_almost_equal(exp_vis, avg.visibilities[orow, ch, corr])
...
def test_dask_averager(
    ...
    da_sigma_spectrum = da.from_array(sigma_spectrum, chunks=(rows, chans, corrs))
    ...
    avg = dask_avg(
        ...
        effective_bw=da_chan_width,
        resolution=da_chan_width,
        weight=da_weight,
        sigma=da_sigma,
        visibilities=da_vis,
        flag=da_flag,
