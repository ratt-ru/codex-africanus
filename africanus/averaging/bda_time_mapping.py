...
RowMapOutput = namedtuple("RowMapOutput",
                          ["map", "time", "interval", "uvw", "flag_row"])


@njit(nopython=True, nogil=True, cache=True)
def row_mapper(time, interval, antenna1, antenna2, uvw,
    ...
    into a `time_mean` array of shape `(ubl, utime)`.
    ...
    time bin in `time_mean`.

    3. The `time_mean` array is flattened and argsorted with a stable
    ...
    and argsorted `time_mean` arrays to an output row.
    ...
    time_mean = np.zeros((nbl, ntime), dtype=time.dtype)
    interval_sum = np.zeros((nbl, ntime), dtype=interval.dtype)
    bl_speed = np.zeros((nl, ntime - 1), dtype=time.dtype)
    u_coord = np.zeros((nbl, ntime), dtype=uvw.dtype)
    ...
    # bin_lookup and time_mean arrays
    for bl in range(ubl.shape[0]):
        ...
        for t in range(utime.shape[0]):
            ...
            time_mean[bl, tbin] += time[r]
            interval_sum[bl, tbin] += interval[r]
            bin_count += 1
        ...
        for b in range(tbin, ntime):
            time_mean[bl, b] = sentinel
            bin_flagged[bl, b] = False

    # Flatten the time lookup and argsort it
    flat_time = time_mean.ravel()
    flat_int = interval_sum.ravel()
    ...
    out_flag_row = None if flag_row is None else np.empty_like(flag_row)
    ...
    for in_row in range(time.shape[0]):
        ...
        if flag_row is not None:
            ...
            if flagged and flag_row[in_row] == 0:
    ...
    uvw_ret = np.zeros((out_rows, 3), dtype=uvw.dtype)

    return RowMapOutput(row_map, time_ret, int_ret, uvw_ret, out_flag_row)
