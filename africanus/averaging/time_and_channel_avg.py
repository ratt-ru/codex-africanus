...
def nb_row_average(
  ...
  def impl(
    ...
    uvw_avg = (
      None if uvw is None else np.zeros((out_rows,) + uvw.shape[1:], dtype=uvw.dtype)
    )

    time_centroid_avg = (
      None
      if time_centroid is None
      else np.zeros((out_rows,) + time_centroid.shape[1:], dtype=time_centroid.dtype)
    )
...
def nb_chan_average(
  ...
  def impl(
    ...
    effective_bw_avg = (
      None if effective_bw is None else np.zeros(out_chans, dtype=effective_bw.dtype)
    )
    ...
    return ChannelAverageOutput(
      chan_freq_avg, chan_width_avg, effective_bw_avg, resolution_avg
    )
