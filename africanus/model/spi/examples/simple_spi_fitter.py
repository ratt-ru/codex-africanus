...
try:
  ...
  def fft(x, ax, ncpu):
    return r2c(x, axes=ax, forward=True, nthreads=ncpu, inorm=0)
  ...
  warnings.warn(
    "No pypocketfft installation found. "
...
def Gaussian2D(xin, yin, GaussPar=(1.0, 1.0, 0.0)):
  S0, S1, PA = GaussPar
  PA = 90 + PA
  SMaj = np.max([S0, S1])
  SMin = np.min([S0, S1])
...
def convolve_model(model, gausskern, args):
  ...
  try:
    ...
    warnings.warn(
      "Could not determine fast fft size. " "Install scipy for optimal performance.",
      ImportWarning,
    )
  ...
  convmodel = fft(iFs(np.pad(model, padding, mode="constant"), axes=ax), ax, args.ncpu)
...
def interpolate_beam(xx, yy, maskindices, freqs, args):
  ...
  if args.beammodel == "eidos":
    ...
    if (
      l_min > l_source.min()
      or m_min > m_source.min()
      or l_max < l_source.max()
      or m_max < m_source.max()
    ):
    ...
    if bfreqs[0] > freqs[0] or bfreqs[-1] < freqs[-1]:
      warnings.warn(
        "The supplied beam does not have sufficient " "bandwidth. Beam frequencies:"
      )
...
def main(args):
  ...
  # load images
  model = np.ascontiguousarray(fits.getdata(args.fitsmodel).squeeze(), dtype=np.float64)
  ...
  if args.fitsresidual is not None:
    ...
    print(
      "Setting cutoff threshold as %i times the rms " "of the residual" % args.threshold
    )
    del resid
  else:
    print(
  ...
  fitcube = da.from_array(
    fitcube.astype(np.float64), chunks=(ncomps // args.ncpu, nband)
  )
  ...
  alpha, _, Iref, _ = fit_spi_components(
    fitcube, weights, freqsdask, np.float64(ref_freq)
  ).compute()
