...
def test_compute_jhj_and_jhr(data_factory):
  ...
  corr_shape = (2,)
  jones_shape = (2,)
  data_dict = data_factory(
    sigma_n, sigma_f, n_time, n_chan, n_ant, n_dir, corr_shape, jones_shape
  )
  time = data_dict["TIME"]
  ...
  jhj1, jhr1 = compute_jhj_and_jhr(
    time_bin_indices, time_bin_counts, ant1, ant2, jones, vis, model, flag
  )
  jhj2 = compute_jhj(time_bin_indices, time_bin_counts, ant1, ant2, jones, model, flag)
  jhr2 = compute_jhr(
    time_bin_indices, time_bin_counts, ant1, ant2, jones, vis, model, flag
  )
...
def test_compute_jhj_dask(data_factory):
  ...
  da_model = da.from_array(
    model, chunks=(row_chunks, (n_chan,), (n_dir,)) + (corr_shape)
  )
...
def test_compute_jhr_dask(data_factory):
  ...
  # get the numpy result
  jhr = np_compute_jhr(
    time_bin_idx, time_bin_counts, ant1, ant2, jones, vis, model, flag
  )
...
def test_phase_only_diag_diag(data_factory):
  ...
  for p in range(n_ant):
    for q in range(p):
      ...
      assert_array_almost_equal(phase_diff_true, phase_diff, decimal=precision - 3)
