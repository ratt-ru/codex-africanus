  ...
  if mode == DIAG_DIAG:
    ...
    def jones_mul(a1j, model, a2j, out):
      ...
      for s in range(n_dir):
        ...
        out[0, 0] += t1 * tmp[0, 0] + t2 * tmp[0, 0] + t3 * tmp[1, 0] + t4 * tmp[1, 0]
        out[0, 1] += t1 * tmp[0, 1] + t2 * tmp[0, 1] + t3 * tmp[1, 1] + t4 * tmp[1, 1]
        t1 = a1j[s, 1, 0] * model[s, 0, 0]
        t2 = a1j[s, 1, 1] * model[s, 1, 0]
        t3 = a1j[s, 1, 0] * model[s, 0, 1]
        t4 = a1j[s, 1, 1] * model[s, 1, 1]
        out[1, 0] += t1 * tmp[0, 0] + t2 * tmp[0, 0] + t3 * tmp[1, 0] + t4 * tmp[1, 0]
        out[1, 1] += t1 * tmp[0, 1] + t2 * tmp[0, 1] + t3 * tmp[1, 1] + t4 * tmp[1, 1]
...
def nb_corrupt_vis(time_bin_indices, time_bin_counts, antenna1, antenna2, jones, model):
...
try:
  corrupt_vis.__doc__ = CORRUPT_VIS_DOCS.substitute(array_type=":class:`numpy.ndarray`")
