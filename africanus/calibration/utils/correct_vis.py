...
def jones_inverse_mul_factory(mode):
  if mode == DIAG_DIAG:
    ...
    def jones_inverse_mul(a1j, blj, a2j, out):
      ...
      out[0, 0] = t1 * b00 + t2 * b00 + t3 * b10 + t4 * b10
      out[0, 1] = t1 * b01 + t2 * b01 + t3 * b11 + t4 * b11
      t1 = a10 * blj[0, 0]
      t2 = a11 * blj[1, 0]
      t3 = a10 * blj[0, 1]
      t4 = a11 * blj[1, 1]
      out[1, 0] = t1 * b00 + t2 * b00 + t3 * b10 + t4 * b10
      out[1, 1] = t1 * b01 + t2 * b01 + t3 * b11 + t4 * b11
...
def correct_vis(
  ...
  return correct_vis_impl(
    time_bin_indices, time_bin_counts, antenna1, antenna2, jones, vis, flag
  )


def correct_vis_impl(
  time_bin_indices, time_bin_counts, antenna1, antenna2, jones, vis, flag
):
...
try:
  correct_vis.__doc__ = CORRECT_VIS_DOCS.substitute(array_type=":class:`numpy.ndarray`")
