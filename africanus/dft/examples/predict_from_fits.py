...
if np.any(ms_freqs != freqs):
  print("Warning - reprojecting fits cube to MS freqs. " "This uses a lot of memory. ")
...
model_predict = np.transpose(model_cube[:, None, idx_nz], [2, 0, 1])
ncomps = idx_nz.size
...
xds = xds_from_ms(
  args.ms, columns=["UVW", args.colname], chunks={"row": args.row_chunks}
)[0]
uvw = xds.UVW.data
