...
def zernike_func(n, m, rho, phi):
    ...
    output_arr[:, 0, 0, (m == 0)[0, :]] = zernike_rad(
        np.zeros(m[m == 0].shape), n[m == 0], rho[:, 0, 0, (m == 0)[0, :]]
    )
    output_arr[:, 0, 0, (m < 0)[0, :]] = zernike_rad(
        -1 * m[m < 0], n[m < 0], rho[:, 0, 0, (m < 0)[0, :]]
    ) * np.sin(-1 * m[m < 0] * phi[:, 0, 0, (m < 0)[0, :]])
    output_arr[:, 0, 0, (m > 0)[0, :]] = zernike_rad(
        m[m > 0], n[m > 0], rho[:, 0, 0, (m > 0)[0, :]]
    ) * np.cos(m[m > 0] * phi[:, 0, 0, (m > 0)[0, :]])
