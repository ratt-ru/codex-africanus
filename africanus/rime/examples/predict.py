...
def load_beams(beam_file_schema, corr_types, l_axis, m_axis):
  ...
  (l_ax, l_grid), (m_ax, m_grid), (nu_ax, nu_grid) = beam_grids(header, l_axis, m_axis)
...
def dde_factory(args, ms, ant, field, pol, lm, utime, frequency):
  ...
  if corr_type_set.issubset(set([9, 10, 11, 12])):
    ...
    raise ValueError(
      "Cannot determine polarisation type "
  ...
  beam, lm_ext, freq_map = load_beams(args.beam, corr_type, args.l_axis, args.m_axis)
...
def vis_factory(args, source_type, sky_model, ms, ant, field, spw, pol):
  ...
  stokes = spectral_model(source.stokes, source.spi, source.ref_freq, frequency, base=0)
