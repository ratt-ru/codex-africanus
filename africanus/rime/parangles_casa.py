...
def casa_parallactic_angles(
  ...
  reference_positions = [
    meas_serv.position("itrf", *(pq.quantity(x, "m") for x in pos))
    for pos in antenna_positions
  ]

  # Compute field centre in radians
  fc_rad = meas_serv.direction("J2000", *(pq.quantity(f, "rad") for f in field_centre))
  ...
  return np.asarray(
    [
      ...
      and [  # Set antenna position as the reference frame
        meas_serv.do_frame(rp) and meas_serv.posangle(fc_rad, zenith).get_value("rad")
