...
def phase_delay(lm, uvw, frequency):
  ...
  out = cp.empty(shape=(lm.shape[0], uvw.shape[0], frequency.shape[0]), dtype=out_dtype)
...
try:
  phase_delay.__doc__ = PHASE_DELAY_DOCS.substitute(array_type=":class:`cupy.ndarray`")
