...
def _key_fn(*args):
  ...
  return tuple(
    (a.dtype, a.ndim) if isinstance(a, (np.ndarray, cp.ndarray)) else a for a in args
  )
...
def predict_vis(
  ...
  if have_ddes:
    ...
    raise ValueError("Insufficient inputs supplied for determining " "the output shape")
  ...
  kernel, block, out_dtype = _generate_kernel(
    time_index,
    antenna1,
    antenna2,
    dde1_jones,
...
try:
  predict_vis.__doc__ = PREDICT_DOCS.substitute(
    ...
    get_time_index=":code:`cp.unique(time, " "return_inverse=True)[1]`",
    extra_args="",
    extra_notes="",
  )
