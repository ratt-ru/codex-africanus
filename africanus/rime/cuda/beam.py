...
def _main_key_fn(
  ...
  return (
    ...
    lm.dtype,
    parangles.dtype,
    pointing_errors.dtype,
    antenna_scaling.dtype,
    frequencies.dtype,
    dde_dims,
    ncorr,
  )
...
def _generate_main_kernel(
  ...
  try:
    ...
    raise ValueError("Number of Correlations not in %s" % list(_corr_shifter.keys()))
...
def beam_cube_dde(
  ...
  kernel, block, dtype = _generate_main_kernel(
    fbeam,
    beam_lm_ext,
    beam_freq_map,
    lm,
...
try:
  beam_cube_dde.__doc__ = BEAM_CUBE_DOCS.substitute(array_type=":class:`cupy.ndarray`")
