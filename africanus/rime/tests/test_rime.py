...
@pytest.mark.parametrize("convention, sign", [("fourier", 1), ("casa", -1)])
def test_phase_delay(convention, sign):
  ...
  frequency = np.linspace(0.856e9, 0.856e9 * 2, 64, endpoint=True)
...
def test_feed_rotation():
  ...
  np_expr = np.stack([pa_cos - 1j * pa_sin, zeros, zeros, pa_cos + 1j * pa_sin], axis=2)
  assert np.allclose(fr, np_expr.reshape(10, 5, 2, 2))
...
def test_dask_phase_delay(convention, sign):
  ...
  dask_phase = dask_phase_delay(
    dask_lm, dask_uvw, dask_frequency, convention=convention
  )
