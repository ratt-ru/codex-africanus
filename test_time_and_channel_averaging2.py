...
vis = (np.arange(time.shape[0]*nchan*ncorr, dtype=np.float32) +
                    np.arange(1, time.shape[0]*nchan*ncorr+1, dtype=np.float32)*1j)
vis = vis.reshape(time.shape[0], nchan, ncorr)

flag = np.random.randint(0, 2, (time.shape[0], nchan, ncorr))
...
print(" uvw \n", uvw)
print(" interval \n", interval)
print(" shape (time, ncorr) \n", shape)
print(" weight \n", weight)
print(" sigma \n", sigma)
print(" shape (shape,nchan,ncorr) \n", shape)
...
flagged_rows =  [] # [], [8, 9], [4], [0, 1]
time_bin_secs = 1 # [1, 2, 3, 4]
chan_bin_size = 1 # [1, 3, 5]
print(" flagged_rows ", flagged_rows)
print(" time_bin_secs ", time_bin_secs)
print(" chan_bin_size ", chan_bin_size)
def test_averager(time, ant1, ant2, flagged_rows,
    ...
    print(uvw.shape, interval.shape, exposure.shape, weight.shape, sigma.shape)
    print(avg.uvw.shape)
...
# Call test_averager with all the data
test_averager(time, ant1, ant2, flagged_rows, uvw, interval, weight, sigma, vis, flag, weight_spectrum, sigma_spectrum, time_bin_secs, chan_bin_size)